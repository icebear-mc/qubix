...
def world_list(request):
    ...
    if owner_id:
        # Show owned worlds + public worlds
        owned = World.objects.filter(owner_id=owner_id)
        public = World.objects.filter(visibility='public')
        worlds = (owned | public).distinct()
    else:
...
def world_create(request):
    ...
    if request.method == 'POST':
        ...
        # Create or get session-based owner ID
        if not request.session.get('owner_id'):
            import uuid
            request.session['owner_id'] = str(uuid.uuid4())
...
def element_create(request, world_pk, element_type):
    ...
    if request.method == 'POST':
        element = WorldElement.objects.create(
            ...
            last_edited_by_id=request.session.get('owner_id')
        )
...
def element_edit(request, pk):
    ...
    if not element.world.can_edit(request):
    ...
    if request.method == 'POST':
        ...
        element.last_edited_by_id = request.session.get('owner_id')
...
def element_detail(request, pk):
    ...
    if not element.world.can_view(request):
        messages.error(request, "You don't have permission to view this element.")
        return redirect('world_list')
    
    can_edit = element.world.can_edit(request)
...
def map_view(request, pk):
    ...
    if not map_entry.world.can_view(request):
